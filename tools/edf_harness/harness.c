 ...
 *
 * Periodic EDF harness for the flow scheduler.
 * Each thread runs periodic jobs with start jitter
...
/* Most start jitter in nanos. Matches the harness range. */
...
static uint64_t uniform_ns(unsigned int *seed, uint64_t lo, uint64_t hi)
{
...
static void *worker_idx(void *arg)
{
    ...
    unsigned int seed = st->seed_base + (unsigned int)idx * 7919U;
    uint64_t jitter;
    uint64_t t0;
    uint64_t wall_end;
    ...
    getrusage(RUSAGE_THREAD, &ru_start);
    {
        ...
        while (now < wall_end) {
            ...
            period = uniform_ns(&seed, HARNESS_PERIOD_MIN_NS, HARNESS_PERIOD_MAX_NS);
            aet = uniform_ns(&seed, HARNESS_AET_MIN_NS, HARNESS_AET_MAX_NS);
    ...
    out->nvcsw = (long)ru_end.ru_nvcsw - (long)ru_start.ru_nvcsw;
    out->nivcsw = (long)ru_end.ru_nivcsw - (long)ru_start.ru_nivcsw;
    return NULL;
}
...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "n:d:s:o:")) != -1) {
        switch (opt) {
        case 'n':
    ...
    st.duration_ns = (uint64_t)(duration_secs * 1000000000.0);
    st.seed_base = seed_base;
    st.outs = calloc((size_t)nthreads, sizeof(struct thread_out));
    wargs = calloc((size_t)nthreads, sizeof(struct worker_arg));
    threads = calloc((size_t)nthreads, sizeof(pthread_t));
    if (!st.outs || !wargs || !threads) {
        fprintf(stderr, "no memory\n");
        return 1;
    }
    pthread_barrier_init(&st.start_bar, NULL, (unsigned)nthreads + 1U);
    for (i = 0; i < nthreads; i++) {
        wargs[i].st = &st;
        wargs[i].idx = i;
        if (pthread_create(&threads[i], NULL, worker_idx, &wargs[i]) != 0) {
    ...
    wall_end = now_ns();
    wall_secs = (double)(wall_end - wall_start) / 1000000000.0;
    if (wall_secs <= 0.0)
    ...
    util_offered = (double)total_released_ns / wall_secs / 1000000000.0 / (double)ncpu;
    if (total_released > 0)
        util_exact = (total_ratio / (double)total_released) * (double)nthreads / (double)ncpu;
    else
        util_exact = 0.0;
    if (total_released > 0)
        completion = (double)total_completed / (double)total_released;
    util_effective = (double)total_completed_ns / wall_secs / 1000000000.0 / (double)ncpu;
    ...
    fprintf(csv, "nthreads,ncpu,wall_secs,released,completed,completion,offered_util,exact_util,effective_util,nvcsw,nivcsw\n");
    fprintf(csv, "%d,%ld,%.3f,%" PRIu64 ",%" PRIu64 ",%.6f,%.6f,%.6f,%.6f,%ld,%ld\n",
        nthreads, ncpu, wall_secs, total_released, total_completed,
        completion, util_offered, util_exact, util_effective, total_nvcsw, total_nivcsw);
    fclose(csv);
    ...
    fprintf(json, "}\n");
    fclose(json);
    printf("done n=%d released=%" PRIu64 " completed=%" PRIu64 " completion=%.4f offered=%.4f exact=%.4f effective=%.4f nvcsw=%ld\n",
        nthreads, total_released, total_completed, completion, util_offered, util_exact, util_effective, total_nvcsw);
